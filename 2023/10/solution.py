...
def east(matrix,  i, j): return matrix[i][j+1] if j < len(matrix[i])-1 else None
def west(matrix,  i, j): return matrix[i][j-1] if j > 0                else None
def south(matrix, i, j): return matrix[i+1][j] if i < len(matrix)      else None
def north(matrix, i, j): return matrix[i-1][j] if i > 0                else None
...
def directions(matrix, i, j):
    ...
    match(pipe):
        case 'F':
            if south(matrix, i, j) in valid_south : directions.append(dsouth)
            if east (matrix, i, j) in valid_east  : directions.append(deast)
        case 'L': 
            if north(matrix, i, j) in valid_north : directions.append(dnorth)
            if east (matrix, i, j) in valid_east  : directions.append(deast)
        case 'J':
            if north(matrix, i, j) in valid_north : directions.append(dnorth)
            if west (matrix, i, j) in valid_west  : directions.append(dwest)
        case '-':
            if west (matrix, i, j) in valid_west  : directions.append(dwest)
            if east (matrix, i, j) in valid_east : directions.append(deast)
        case '|':
...
def dfs_iterative(matrix, start_i, start_j):
    ...
    while stack:
        ...
        if len(d) < 2: continue                 # no pipe continuing
        if prec and prec in d: d.remove(prec)   # removing source pipe as direction
...
def count_board(matrix):
    ...
    for i, row  in enumerate(matrix):
        ...
        for j, cell in enumerate(row):
            ...
            match cell:
                case 'F':
                    if not opened: opened = cell
                    elif opened == 'F' or opened == '|' or opened == '7': opened = False

                case '|':
                    if not opened: opened = cell 
                    elif opened == '|' or opened =='7': opened = False

                case '7':
                    if not opened: opened = cell 
                    elif opened == 'F' or opened == '|': opened = False
                # case 'J':
